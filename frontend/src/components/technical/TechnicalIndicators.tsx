      ...
      <div className="p-4 space-y-4">
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            <IndicatorBox
              label="SMA 20"
              value={data.moving_averages?.sma_20 ? `$${data.moving_averages.sma_20.toFixed(2)}` : 'N/A'}
              signal={data.moving_averages?.sma_20 && data.current_price > data.moving_averages.sma_20 ? 'bullish' : 'neutral'}
            />
            <IndicatorBox
              label="SMA 50"
              value={data.moving_averages?.sma_50 ? `$${data.moving_averages.sma_50.toFixed(2)}` : 'N/A'}
              signal={data.moving_averages?.sma_50 && data.current_price > data.moving_averages.sma_50 ? 'bullish' : 'neutral'}
            />
            <IndicatorBox
              label="SMA 200"
              value={data.moving_averages?.sma_200 ? `$${data.moving_averages.sma_200.toFixed(2)}` : 'N/A'}
              signal={data.moving_averages?.sma_200 && data.current_price > data.moving_averages.sma_200 ? 'bullish' : 'neutral'}
            />
          </div>
        </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 rounded-lg p-2.5 text-center dark:from-green-900/20 dark:to-emerald-900/20 dark:border-green-800">
                ...
                <p className="font-bold text-green-700 dark:text-green-300">${data.support_resistance.support_1?.toFixed(2) ?? 'N/A'}</p>
              </div>
              <div className="bg-gradient-to-br from-gray-50 to-slate-50 border border-gray-200 rounded-lg p-2.5 text-center dark:from-gray-700/50 dark:to-slate-700/50 dark:border-gray-600">
                ...
                <p className="font-bold text-gray-800 dark:text-gray-200">${data.support_resistance.pivot?.toFixed(2) ?? 'N/A'}</p>
              </div>
              <div className="bg-gradient-to-br from-red-50 to-rose-50 border border-red-200 rounded-lg p-2.5 text-center dark:from-red-900/20 dark:to-rose-900/20 dark:border-red-800">
                ...
                <p className="font-bold text-red-700 dark:text-red-300">${data.support_resistance.resistance_1?.toFixed(2) ?? 'N/A'}</p>
              </div>
            </div>
          </div>
        )}
        ...
        {/* MACD */}
        {data.macd && (data.macd.macd_line != null || data.macd.signal_line != null) && (
          <div>
            ...
            <div className="grid grid-cols-3 gap-2 text-sm">
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-2.5 text-center dark:bg-gray-700/50 dark:border-gray-600">
                ...
                <p className={`font-bold ${(data.macd.macd_line ?? 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                  {data.macd.macd_line?.toFixed(4) ?? 'N/A'}
                </p>
              </div>
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-2.5 text-center dark:bg-gray-700/50 dark:border-gray-600">
                <p className="text-xs text-gray-500 dark:text-gray-400">Signal</p>
                <p className="font-bold text-gray-700 dark:text-gray-300">{data.macd.signal_line?.toFixed(2) ?? 'N/A'}</p>
              </div>
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-2.5 text-center dark:bg-gray-700/50 dark:border-gray-600">
                <p className="text-xs text-gray-500 dark:text-gray-400">Histogram</p>
                <p className={`font-bold ${(data.macd.histogram ?? 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
